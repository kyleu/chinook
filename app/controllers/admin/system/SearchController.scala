...
class SearchController @javax.inject.Inject() (override val app: Application, services: ServiceRegistry) extends BaseController("search") {
  ...
  private[this] def searchString(creds: Credentials, q: String)(implicit timing: TraceData) = {
    ...
    val album = services.mediaServices.albumService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.media.albumSearchResult(model, s"Album [${model.albumId}] matched [$q].")
    })
    val artist = services.mediaServices.artistService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.media.artistSearchResult(model, s"Artist [${model.artistId}] matched [$q].")
    })
    val auditRecord = services.auditServices.auditRecordService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.audit.auditRecordSearchResult(model, s"Audit Record [${model.id}] matched [$q].")
    })
    val customer = services.commerceServices.customerService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.commerce.customerSearchResult(model, s"Customer [${model.customerId}] matched [$q].")
    })
    val employee = services.commerceServices.employeeService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.commerce.employeeSearchResult(model, s"Employee [${model.employeeId}] matched [$q].")
    })
    ...
    val genre = services.mediaServices.genreService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.media.genreSearchResult(model, s"Genre [${model.genreId}] matched [$q].")
    })
    val invoice = services.commerceServices.invoiceService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.commerce.invoiceSearchResult(model, s"Invoice [${model.invoiceId}] matched [$q].")
    })
    val invoiceLine = services.commerceServices.invoiceLineService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.commerce.invoiceLineSearchResult(model, s"Invoice Line [${model.invoiceLineId}] matched [$q].")
    })
    val mediaType = services.mediaServices.mediaTypeService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.media.mediaTypeSearchResult(model, s"Media Type [${model.mediaTypeId}] matched [$q].")
    })
    val note = services.noteServices.noteService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.note.noteSearchResult(model, s"Note [${model.id}] matched [$q].")
    })
    val playlist = services.mediaServices.playlistService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.media.playlistSearchResult(model, s"Playlist [${model.playlistId}] matched [$q].")
    })
    val playlistTrack = services.mediaServices.playlistTrackService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.media.playlistTrackSearchResult(model, s"Playlist Track [${model.playlistId}, ${model.trackId}] matched [$q].")
    })
    ...
    val track = services.mediaServices.trackService.searchExact(creds, q = q, limit = Some(5)).map(_.map { model =>
      views.html.admin.media.trackSearchResult(model, s"Track [${model.trackId}] matched [$q].")
    })

    val stringSearches = Seq[Future[Seq[Html]]](album, artist, customer, employee, genre, invoice, invoiceLine, mediaType, playlist, playlistTrack, track, auditRecord, flywaySchemaHistory, note, scheduledTaskRun, syncProgress, systemUser)
    // End string searches
